...
<header>
  ...
  <nav>
    <ul class="nav">
      ...
      <li><a href="adoptable-cats.php" class="home-btn">Adopt Cats</a></li>
...
<section class="AdoptableHero">
  <div class="AdoptableHero-content">
    <h1>Adoptable Cats Near You</h1>
    <p>Find your furever friend today!</p>
  </div>
</section>
...
<main class="container">
  ...
  <section class="search-section">
    <div class="search-bar">
      <input type="text" class="search-input" placeholder="Search by breed, age..." id="searchInput">
...
<?php
$sql = "SELECT a.AnnouncementID, a.Title, a.Description, a.PhotoURL, a.Location, a.ContactPhone, u.Username, c.Age, c.Gender, c.Vaccinated, c.Neutered
        FROM Announcement a
...
<a href="view-announcement.php?id='. $row['AnnouncementID'] .'"
   ...
   data-breed="'. strtolower($row['Title']) .'"
   data-age="'. strtolower($row['Age']) .'">
  ...
  <div class="cat-card">
    ...
    <div class="cat-details">
      ...
      <p><strong>Vaccinated:</strong> '. htmlspecialchars($row['Vaccinated']) .'</p>
      <p><strong>Neutered:</strong> '. htmlspecialchars($row['Neutered']) .'</p>
    ...
    echo '<section class="empty-state">
            <div class="empty-content">
                ...
                <h2>No Cats Available for Adoption</h2>
                <p>There are currently no cats available for adoption. Be the first to post an adoption announcement!</p>
                <button class="cta-button" onclick="location.href=\'add.php\'">Post an Adoption Announcement</button>
            </div>
          </section>';
}
?>
...
function filterPosts() {
  ...
  posts.forEach(post => {
    ...
    const postBreed = post.dataset.breed;
    const postAge = post.dataset.age;

    const matchesSearch = postBreed.includes(searchText) || postAge.includes(searchText);
